...
\subsection{A multi-engine synthetic corpus}
\label{sec:multiengine}

The narrow corpus above exposes the gap; a broader corpus narrows it. We build a
second training corpus that varies the two axes the first one fixes, speaker and
channel. Three TTS systems render 300 code-switch transcripts each: edge-tts
multilingual voices (one bilingual voice per utterance, so the switch stays inside a
single speaker), Kokoro, and MeloTTS. The transcripts span a switching spectrum,
from one English word in a long Japanese sentence to clause-level alternation, and
each is tagged with its density and span type. Rendering yields 900 clean clips at
\SI{16}{\kilo\hertz} mono, 300 per engine.

We then expand each clean clip into five channel conditions: MUSAN~\cite{snyder2015musan}
noise, music, and multi-speaker babble at swept SNR; room reverberation convolved
from recorded impulse responses; and a G.711 telephony round-trip. The full corpus
holds 5,400 clips. Section~\ref{sec:forgetting} and the real-transfer ablation
(Table~\ref{tab:real-ablation}) report what the added speaker and channel variation
buys on real speech: it is the difference between \SI{67}{\percent} and
\SI{21}{\percent} MER on CS-FLEURS.
...
\subsection{What transfers to real speech}
\label{sec:realtransfer}

The synthetic ablations above rank conditions by in-domain fit. On real speech the
ranking changes. We retrain Whisper large-v3 on the multi-engine corpus under three
variations and score each on CS-FLEURS (Table~\ref{tab:real-ablation}).

\begin{table}[t]
\centering
\caption{Real-transfer ablation (Whisper large-v3, MER). The condition that wins on
synthetic audio, clean-only training, loses on real speech.}
\label{tab:real-ablation}
\small
\begin{tabular}{lrr}
\toprule
Condition & CS-FLEURS MER (\%) & Synthetic MER (\%) \\
\midrule
Full corpus ($r{=}8$)      & \best{21.1} & 6.0 \\
Clean only (no augment.)   & 67.3 & \best{3.5} \\
Single engine              & 33.5 & 4.5 \\
LoRA $r{=}16$              & 26.2 & 4.4 \\
LoRA $r{=}32$              & 26.6 & 4.7 \\
\bottomrule
\end{tabular}
...
Augmentation dominates transfer. Training on clean audio alone leaves CS-FLEURS MER
at \SI{67.3}{\percent}, while the full augmented corpus reaches \SI{21.1}{\percent},
a threefold difference on real speech. Clean-only is the \emph{best} model on
synthetic audio (\SI{3.5}{\percent} MER against the full corpus's
\SI{6.0}{\percent}): the condition that wins in-domain loses on transfer, so a
synthetic leaderboard would select the worst real-speech model. Engine diversity is
the second lever, one engine reaching \SI{33.5}{\percent} against three engines'
\SI{21.1}{\percent}. LoRA rank barely moves the result, and higher rank slightly
hurts (\num{21.1}, \num{26.2}, \num{26.6} for $r{=}8,16,32$), consistent with mild
overfitting to the synthetic distribution.

\subsection{Monolingual forgetting and its cure}
\label{sec:forgetting}

Fine-tuning on code-switching audio degrades monolingual Japanese, and monolingual
replay reverses the loss. We retrain Whisper large-v3 on a three-engine synthetic
corpus and evaluate on the FLEURS \texttt{ja\_jp} and \texttt{en\_us} test sets,
which share CS-FLEURS's sentence domain and speaker pool. The matched controls
therefore isolate the fine-tune as the cause of any monolingual regression.
Table~\ref{tab:forgetting} compares the base model, the code-switching-only
fine-tune, and a fine-tune that adds \num{2000} monolingual FLEURS clips
(\num{1000} Japanese, \num{1000} English) to the training mix.

\begin{table}[t]
\centering
\caption{Monolingual forgetting and replay (Whisper large-v3). Code-switching-only
fine-tuning more than triples Japanese CER; adding monolingual clips restores it and
lowers CS-FLEURS error.}
\label{tab:forgetting}
\small
\begin{tabular}{lrrr}
\toprule
Model & JA-CER (\%) & EN-WER (\%) & CS-FLEURS MER (\%) \\
\midrule
Base large-v3            & \best{6.0} & \best{4.2} & 40.9 \\
{+}\,LoRA, CS only       & 20.5 & 4.7 & 21.1 \\
{+}\,LoRA, CS {+} mono   & \best{6.0} & 4.9 & \best{17.0} \\
\bottomrule
\end{tabular}
\end{table}

Code-switching fine-tuning raises Japanese CER from \SI{6.0}{\percent} to
\SI{20.5}{\percent}, more than tripling it, while English WER moves from
\SI{4.2}{\percent} to \SI{4.7}{\percent}. The asymmetry follows from the corpus.
Its English insertions are plentiful and its Japanese matrix is short, so the
adapter overwrites Japanese acoustics first. Every smaller model shows the same
regression, and Whisper Small forgets most (JA-CER \SI{14.1}{\percent} to
\SI{42.7}{\percent}).

Monolingual replay restores Japanese to its pre-fine-tuning CER of
\SI{6.0}{\percent} and holds English at \SI{4.9}{\percent}. Replay also improves
the target task: CS-FLEURS MER drops from \SI{21.1}{\percent} to
\SI{17.0}{\percent}, because CS-FLEURS utterances are \SI{41}{\percent} Japanese
characters and stronger Japanese transcription helps the matrix language of each
sentence. Two thousand clips from an existing corpus buy both monolingual retention
and higher code-switching accuracy.
...
We score both settings with MER under the same decoding, so the two numbers compare
directly. The table reports the merged checkpoint and its zero-shot base on the
reconstructed synthetic CS test split ($n=30$) and on CS-FLEURS ($n=196$).
\begin{center}
\begin{tabular}{lrr}
\toprule
Setting & Zero-shot MER & {+}\,LoRA MER \\
\midrule
Synthetic CS test split ($n=30$) & 26.6 & \best{1.1} \\
CS-FLEURS JA--EN ($n=196$)       & 40.9 & 26.1 \\
\bottomrule
\end{tabular}
\end{center}
...
\caption{The same merged checkpoint on two test sets, scored with MER on both:
\SI{1.1}{\percent} on the synthetic split against \SI{26.1}{\percent} on CS-FLEURS.
The in-domain number reads as solved; the real number remains far from it.}
% TODO regenerate fig_gap.pdf as a single MER comparison via make_figures.py;
% the committed PDF still shows the earlier relative-gain panels.
\label{fig:gap}
\end{figure}

The same checkpoint scores \SI{1.1}{\percent} MER on the synthetic split and
\SI{26.1}{\percent} on CS-FLEURS, a \num{24}-fold gap on one metric. Fine-tuning
cuts synthetic MER \num{24}-fold (\num{26.6} to \num{1.1}) but real MER by a third
(\num{40.9} to \num{26.1}). Most of the in-domain improvement fits the synthesiser
and does not survive a change of speaker and channel. On ScriptAcc the fine-tuned
checkpoint reaches \SI{80.5}{\percent} against zero-shot Whisper's
\SI{64.6}{\percent}, a \num{15.9}-point gain, and it still drops Latin script for a
fifth of English reference words.
...
We draw two conclusions for practice. First, \textbf{a synthetic held-out split
measures only fit to the synthesiser}; any claim about deployment needs real
speech, even a small quantity of it. The same checkpoint that
reads as solved on synthetic audio (\SI{1.1}{\percent} MER) leaves a quarter of real
speech wrong (\SI{26.1}{\percent} MER). Second, \textbf{script accuracy names the
failure that word error rate hides}. The fine-tuned checkpoint keeps
\SI{100}{\percent} of English in Latin script on synthetic audio but
\SI{80.5}{\percent} on real, and the missing fifth is katakana substitution, which
makes a transcript unusable downstream while barely moving WER.
